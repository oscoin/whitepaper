...
$x$. If $x \in \projs$ then the transaction is valid if the contract at slot
$\mathtt{validTransfer}$ asserts that it is (see \ref{s:smart-contracts}). The
default will require at a signature from all owners. In any case, for the
transaction to be valid $\bal(x) \geq n$ must hold, and after the transaction
$x$'s balance is decremented by $n$ and $y$'s is incremented by $n$.
