...
address, and $n$ is the amount of transferred \oscoin{}. The transaction is
valid so long as $\bal(x) \geq n$ holds and the contracts associated with $x$ and
...
\subsubsection{Registration and ownership} Projects must be
...
the account associated with $k_1$. This helps to ensure abandonned projects
...
% TODO: What is the incentive for maintainers to keep the checkpoint up to date,
% particularly if it costs money to make a transaction?
...
aggregated checkpoint must also be signed by a key in the project key set.
...
entire project. When cross-referenced with the project's repository, this constitutes
a complete historical record of who authored what code, and which
maintainer signed off on the contribution. This ensutes the project history is auditable and
tamper-proof, while providing fundemental information for \osrank{}
computation. Note that only contribution \emph{metadata} is stored on-chain.
...
\oscoin{} should map one-to-one with Nix
dependencies. Dependencies are an important input to the \osrank{}
metric.

% TODO: This last sentence seems self-evident at this point.
